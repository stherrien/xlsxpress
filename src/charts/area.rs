...
///
/// Creates area charts with support for multiple data series, titles, legends,
/// and positioning. Area charts display data as filled areas under lines.
///
/// # Examples
///
/// ```rust,ignore
/// use xlsxpress::charts::{AreaChart, DataSeries};
///
/// let chart = AreaChart::new()
///     .title("Revenue Over Time")
///     .add_series(DataSeries::new("Sheet1!$B$2:$B$13")
///         .name("Revenue")
...
mod tests {
    ...
    fn test_area_chart_with_title() {
        let chart = AreaChart::new().title("Revenue Trends");
        assert_eq!(Chart::title(&chart), Some("Revenue Trends"));
    }
    ...
    fn test_area_chart_with_axis_titles() {
        ...
        assert_eq!(chart.get_x_axis_title(), Some("Time Period"));
        assert_eq!(chart.get_y_axis_title(), Some("Amount ($)"));
    }

    /// TDD RED: Test area chart with series
    #[test]
    fn test_area_chart_with_series() {
        let series = DataSeries::new("Sheet1!$B$2:$B$13")
            .name("Monthly Revenue")
            .categories("Sheet1!$A$2:$A$13");
        let chart = AreaChart::new().add_series(series);

        assert_eq!(chart.get_series().len(), 1);
        assert_eq!(chart.get_series()[0].get_name(), Some("Monthly Revenue"));
    }
    ...
    fn test_area_chart_with_multiple_series() {
        let chart = AreaChart::new()
            .add_series(
                DataSeries::new("Sheet1!$B$2:$B$13")
                    .name("Revenue")
                    .categories("Sheet1!$A$2:$A$13"),
            )
            .add_series(
                DataSeries::new("Sheet1!$C$2:$C$13")
                    .name("Costs")
                    .categories("Sheet1!$A$2:$A$13"),
            );
        ...
        assert_eq!(chart.get_series()[1].get_name(), Some("Costs"));
    }

    /// TDD RED: Test area chart with position
    #[test]
    fn test_area_chart_with_position() {
        let pos = ChartPosition::new(1, 1).width(720).height(480);
        ...
        assert_eq!(chart_pos.width, Some(720));
    ...
    fn test_area_chart_stacked() {
        let chart = AreaChart::new().stacked(true);
        assert!(chart.is_stacked());

        let chart = AreaChart::new().stacked(false);
        assert!(!chart.is_stacked());
    }
    ...
    fn test_area_chart_builder() {
        ...
        assert_eq!(Chart::title(&chart), Some("Quarterly Performance"));
        assert_eq!(chart.get_x_axis_title(), Some("Quarter"));
        assert_eq!(chart.get_y_axis_title(), Some("Value"));
        assert_eq!(chart.get_series().len(), 2);
        assert!(chart.is_stacked());
        assert!(chart.is_legend_shown());
    }
